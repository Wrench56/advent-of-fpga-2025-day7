...
let testbench ~saturating =
  ...
  in
  let rec wait_for_ready = function
    | times when times >= max_cycles ->
      failwith "Max cycles reached while waiting for Accumulator's ready signal"
    | times ->
      if Bits.equal !(outputs.ready) Bits.gnd
      then (
        cycle 1;
        wait_for_ready (times + 1))
      else ()
  in
  let add num =
